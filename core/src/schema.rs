...
pub struct Column {
    ...
    /// La colonna è auto-generata (identity/serial/AUTO_INCREMENT/…)?
    #[serde(default)]
    pub auto_increment: bool,
    /// Espressione/letterale di default della sorgente (grezzo), se presente.
    #[serde(default)]
    pub default: Option<String>,
}

/// Un indice (non-PK) nel modello neutro.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Index {
    pub name: String,
...
/// Una foreign key nel modello neutro.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct Table {
    ...
    /// Indici non-PK (per la clonazione cross-motore).
    #[serde(default)]
    pub indexes: Vec<Index>,
    /// Foreign key (aggiunte in coda alla migrazione).
    #[serde(default)]
...
/// Sintassi di auto-incremento per il motore target (vuoto per SQLite, dove è
/// gestito dall'alias di rowid `INTEGER PRIMARY KEY`).
fn auto_increment_ddl(engine: Engine) -> &'static str {
    match engine {
        Engine::Mysql => " AUTO_INCREMENT",
        Engine::Sqlserver => " IDENTITY(1,1)",
        Engine::Postgres | Engine::Oracle => " GENERATED BY DEFAULT AS IDENTITY",
        Engine::Sqlite => "",
    }
}

/// Traduce (best-effort) un default della sorgente per il target. Le espressioni
/// temporali comuni diventano il "now" del target; sequenze/funzioni ignote si
/// saltano (ritorna None); i letterali passano invariati.
fn default_ddl(raw: &str, engine: Engine) -> Option<String> {
    let low = raw.trim().trim_matches('"').to_lowercase();
    let now = [
        "now()", "current_timestamp", "current_timestamp()", "getdate()", "sysdate", "systimestamp",
    ];
    if now.iter().any(|x| low == *x) {
        return Some(match engine {
            Engine::Oracle => "SYSTIMESTAMP".into(),
            _ => "CURRENT_TIMESTAMP".into(),
        });
    }
    // Sequenze o chiamate di funzione non traducibili → nessun default.
    if low.contains("nextval") || low.contains('(') {
        return None;
    }
...
/// colonne (tipo + default/auto-increment + nullabilità) + eventuale PRIMARY KEY.
pub fn create_table_ddl(t: &Table, engine: Engine) -> String {
    let mut defs: Vec<String> = t
        .columns
        .iter()
        .map(|c| {
            let mut d = format!("  {} {}", quote_ident(engine, &c.name), c.ty.to_ddl(engine));
            if c.auto_increment {
                d.push_str(auto_increment_ddl(engine));
            } else if let Some(def) = &c.default {
                if let Some(dd) = default_ddl(def, engine) {
                    d.push_str(&format!(" DEFAULT {dd}"));
                }
            }
            if !c.nullable {
                d.push_str(" NOT NULL");
...
/// `CREATE [UNIQUE] INDEX` per il target.
pub fn index_ddl(table: &str, idx: &Index, engine: Engine) -> String {
    let uniq = if idx.unique { "UNIQUE " } else { "" };
    let cols = idx.columns.iter().map(|c| quote_ident(engine, c)).collect::<Vec<_>>().join(", ");
    format!(
        "CREATE {uniq}INDEX {} ON {} ({cols});",
        quote_ident(engine, &idx.name),
        quote_ident(engine, table)
    )
}

/// `ALTER TABLE … ADD FOREIGN KEY` per il target (da emettere in coda, dopo i dati).
pub fn fk_ddl(table: &str, fk: &ForeignKey, engine: Engine) -> String {
    let cols = fk.columns.iter().map(|c| quote_ident(engine, c)).collect::<Vec<_>>().join(", ");
    let rcols = fk.ref_columns.iter().map(|c| quote_ident(engine, c)).collect::<Vec<_>>().join(", ");
    format!(
...
mod tests {
    ...
    fn tbl(name: &str, columns: Vec<Column>) -> Table {
        Table { name: name.into(), columns, indexes: vec![], foreign_keys: vec![] }
    }
